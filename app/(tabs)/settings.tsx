...
export default function Settings() {
	...
	const [sonarrAddress, setSonarrAddress] = useState<any>({ prev: "", new: "" });
	const [radarrAddress, setRadarrAddress] = useState<any>({ prev: "", new: "" });
		...
		>
			<ScrollView>
				<View className="flex flex-col gap-3">
					...
					<View className="px-3 py-2 rounded-lg" style={{ borderWidth: 1, borderColor: "#4b4c4c" }}>
						<Text className="text-sm text-white">
							Sonarr
						</Text>
						<ThemedTextInput placeholder='Sonarr Address' onChangeText={(e) => {
							setFormHasChanged(e !== sonarrAddress.prev)
							setSonarrAddress({ prev: sonarrAddress.prev, new: e });
						}} value={sonarrAddress.new} />
						<ThemedTextInput placeholder='Sonarr Api Key' onChangeText={(e) => {
							setFormHasChanged(e !== sonarrApiKey.prev)
							setSonarrApiKey({ prev: sonarrApiKey.prev, new: e });
					...
					<View className="px-3 py-2 rounded-lg" style={{ borderWidth: 1, borderColor: "#4b4c4c" }}>
						...
						<ThemedTextInput placeholder='Radarr Address' onChangeText={(e) => {
							setFormHasChanged(e !== radarrAddress.prev)
							const updateRadarrAddress = { prev: radarrAddress.prev, new: e };
							setRadarrAddress(updateRadarrAddress)
						}} value={radarrAddress.new} />
						<ThemedTextInput placeholder='Radarr Api Key' onChangeText={(e) => {
							setFormHasChanged(e !== radarrApiKey.prev)
							setRadarrApiKey({ prev: radarrApiKey.prev, new: e });
						}} value={radarrApiKey.new} />
					</View>
				</View>
				<View className="flex flex-col gap-3 mt-10">
					<Pressable className="bg-red-700 max-w-32 px-2 py-3 items-center rounded-lg" android_ripple={{ color: "red" }} onPress={() => {
						Alert.alert(
							'Confirm',
					...
					<Pressable className="bg-red-700 max-w-52 px-2 py-3 items-center rounded-lg" android_ripple={{ color: "red" }} onPress={async () => {
						Alert.alert(
							'Confirm',
							'Are you sure you want to proceed? This will logout all logged in devices!',
							[
								{ text: 'Cancel', style: 'cancel' },
								{ text: 'Logout all devices', onPress: logoutall },
							],
						);
					}}>
			...
			{formHasChanged ? (
				<SaveButton callback={updateData} label="Save" className="absolute bottom-20 right-4" />
			) : null}
		</KeyboardAvoidingView>
	)
}
