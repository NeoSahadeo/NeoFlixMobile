		...
		>
			...
			<View className='flex w-full px-10 gap-3 mb-10'>
				...
				<Controller
					...
					name="username"
					render={({ field: { onChange, onBlur, value } }) => (
						<ThemedTextInput placeholder='Username' onChangeText={onChange} onBlur={onBlur} value={value} />
					)}
				/>
				<Controller
					control={control}
					name="password"
					render={({ field: { onChange, onBlur, value } }) => (
						<ThemedTextInput placeholder='Password' onChangeText={onChange} onBlur={onBlur} value={value} className='mb-10' secureTextEntry={false} />
					)}
				/>
				<Pressable className='px-10 py-5 bg-blue-500 rounded w-auto flex items-center' android_ripple={{ color: "cyan" }} onPress={handleSubmit(login)}>
